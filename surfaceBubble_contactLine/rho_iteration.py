def rho_iteration(box_x, box_y, box_z, rho, T, phi, mu, k, e_ff):
    ...
    rho_expend[1:box_x+1, 1:box_y+1, 0] = rho[:, :, 0]  # mirror boundary condition

    rho_expend[box_x+1, 1:box_y+1, 1:box_z+1] = rho[0, :, :]
    rho_expend[1:box_x+1, box_y+1, 1:box_z+1] = rho[:, 0, :]
    rho_expend[1:box_x+1, 1:box_y+1, box_z+1] = rho[:, :, box_z-1] # mirror boundary condition
    ...
    rho_new = 1/(1 + np.exp(-belta * (e_ff * rho_sumption - phi + mu + k*surface*delta_function.delta(rho))))
